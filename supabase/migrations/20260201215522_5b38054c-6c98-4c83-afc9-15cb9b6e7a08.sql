-- Function to recalculate fidelity stats for all clients
  ...
  LOOP
    ...
    LOOP
      ...
      IF v_appointments.total_price >= COALESCE(v_unit.fidelity_min_value, 0)
         AND v_appointments.payment_method != 'Cortesia de Fidelidade' THEN
        v_loyalty_cuts := v_loyalty_cuts + 1;
      END IF;
    END LOOP;
    
    -- Calculate earned courtesies and current cuts
    v_earned_courtesies := FLOOR(v_loyalty_cuts / COALESCE(v_unit.fidelity_cuts_threshold, 10));
...
GRANT EXECUTE ON FUNCTION recalculate_all_client_fidelity() TO authenticated;
